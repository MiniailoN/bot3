...
def delete_expired():
    ...
    c.execute("DELETE FROM qr_codes WHERE expiration_date <= ?", (now,))
    conn.commit()
    conn.close()
...
async def select_qr(query: types.CallbackQuery, state: FSMContext):
    ...
    if qr_codes:
        ...
        await query.message.answer("У вас пока нет сохранённых QR-кодов.", reply_markup=start_kb())
...
async def type(query: types.CallbackQuery, state: FSMContext):
    ...
    await query.message.answer(f"Вы выбрали тип: {data_type}. Введите данные:")
...
async def data(msg: types.Message, state: FSMContext):
    ...
    await msg.answer("Выберите срок хранения QR-кода:", reply_markup=exp_kb())
...
async def exp(query: types.CallbackQuery, state: FSMContext):
    ...
    await query.message.answer("Введите название для QR-кода:")
    await state.set_state(QRState.name)

@dp.message(QRState.name)
async def name(msg: types.Message, state: FSMContext):
    user_data = await state.get_data()
    qr_name, qr_data, exp_date = msg.text, user_data.get("data", ""), user_data.get("exp", "")
    ...
    await msg.answer_photo(photo, caption=f"Ваш QR-код '{qr_name}' готов!", reply_markup=start_kb())
    os.remove(img_path)
    await state.clear()
